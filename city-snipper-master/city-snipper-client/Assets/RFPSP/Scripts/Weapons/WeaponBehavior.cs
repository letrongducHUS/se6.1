   ...
   int timetosmooth=1;
	...
	void Update(){
        ...
        if (isbulletmove)
        {
            ...
            {
                if (timetosmooth >= 1 && timetosmooth <= 50)
                {
                    Time.timeScale = 0.2f;
                }
                else
                ...
                timetosmooth++;
                FPSPlayer.instance.ZoomScope.SetActive(false);
            }
	...
	void HitObject ( RaycastHit hit, Vector3 directionArg, bool isSecondCast = false ){
        ...
        {
            if (EnemyLeft.instance.myenemiesleft == 1)
            {
                print("RFPSEnemyLeft");
                isbulletmove = true;
                NewBullet = Instantiate(BulletPrefab, BulletSpawnPoint.transform.position, BulletSpawnPoint.transform.rotation);
                directionhitpoint = hit.point;
                hit.transform.gameObject.GetComponentInParent <CharacterDamage>().AIComponent.walkSpeed=0 ;
        ...
        if (hit.transform.gameObject.layer == LayerMask.NameToLayer("Enemy"))
        {
            if (EnemyLeft.instance.myenemiesleft == 1)
            {
                ...
                hit.transform.gameObject.GetComponent<DarkTreeFPS.NPC>().navMeshAgent.isStopped=true;

            }
            else
            {
                isbulletmove = false;
                hit.transform.gameObject.GetComponent<DarkTreeFPS.NPC>().GetHit(damage);
            }
            
        } 
        if (hit.transform.gameObject.layer == LayerMask.NameToLayer("Enemy1"))
        {
            if (EnemyLeft.instance.myenemiesleft == 1)
            {
                isbulletmove = true;
        ...
        if (hit.transform.gameObject.layer == LayerMask.NameToLayer("Enemy2"))
        {
            if (EnemyLeft.instance.myenemiesleft == 1)
            {
                isbulletmove = true;
        ...
        if (hit.transform.gameObject.layer == LayerMask.NameToLayer("Enemy3"))
        {
            if (EnemyLeft.instance.myenemiesleft == 1)
            {
                isbulletmove = true;
        ...
        if (hit.transform.gameObject.layer == LayerMask.NameToLayer("Enemy4"))
        {
            if (EnemyLeft.instance.myenemiesleft == 1)
            {
                isbulletmove = true;
    ...
    public void Ajudst(float value)
    {
        zoomFOV = value;
    }
