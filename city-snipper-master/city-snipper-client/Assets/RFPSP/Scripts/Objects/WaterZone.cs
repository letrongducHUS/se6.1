			...
			if(FPSWalkerComponent.eyePos.y <= myTransform.GetComponent<Collider>().bounds.max.y){
				...
				if(FPSWalkerComponent.inputY == 0 && FPSWalkerComponent.inputX == 0){//player is treading water
					...
					if(rippleEmitTime + rippleEmitDelay < Time.time){
						rippleEffect.Emit(Mathf.RoundToInt(rippleEffect.emissionRate));//emit the particle(s)
					...
					if(splashTrailTime + 0.075f < Time.time){
						...
						splashTrail.Emit(Mathf.RoundToInt(splashTrail.emissionRate));//emit the particle(s)
