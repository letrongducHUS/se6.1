...
namespace DarkTreeFPS
{
    public class Weapon : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (Input.GetKey(input.Fire) &&!InputManager.useMobileInput && !PlayerStats.isPlayerDead && weaponType != WeaponType.Pistol && !InventoryManager.showInventory && fireMode == FireMode.automatic)  //Statement to restrict auto-fire for pistol weapon type. Riffle and others are automatic
            {
                Fire();
            }
            else if (Input.GetKeyDown(input.Fire) && !InputManager.useMobileInput && !PlayerStats.isPlayerDead && (weaponType == WeaponType.Pistol || fireMode == FireMode.single) && !InventoryManager.showInventory)
            {
                Fire();
            }
