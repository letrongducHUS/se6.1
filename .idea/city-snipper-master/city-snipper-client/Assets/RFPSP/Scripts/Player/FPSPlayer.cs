...
public class FPSPlayer : MonoBehaviour {
    ...
    public GameObject ZoomScope;

    float TemporaryHealth;
    public Text TotalHealthLefts;
    void Start (){
        instance = this;
		if(removePrefabRoot){
...
//		llf.GetComponent<LevelLoadFade>().FadeAndLoadLevel(Color.black, 2.0f, true);
	...
	void LateUpdate () {
        TotalHealthLefts.text = PlayerPrefs.GetInt("GiveHealth").ToString();
...
//	}
    ...
    {
        if (hitPoints < TemporaryHealth)
        {
            int newhealth1 = PlayerPrefs.GetInt("GiveHealth");
            if (newhealth1 > 0)
            {
                hitPoints = TemporaryHealth;
                int newhealth = PlayerPrefs.GetInt("GiveHealth");
                newhealth -= 1;
                PlayerPrefs.SetInt("GiveHealth", newhealth);
            }
        }
    }
